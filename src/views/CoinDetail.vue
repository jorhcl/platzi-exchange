    ...
    <template v-if="!isLoading">
      <div class="flex flex-col sm:flex-row justify-around items-center">
        ...
        <div class="my-10 sm:mt-0 flex flex-col justify-center text-center">
          ...
          <span class="text-xl">{{ convetResult}}</span>
        </div>
      ...
      <table>
        <tr v-for="m in markets" :key="`${m.exchangeId}-${m.priceUsd}`" class="border-b">
          ...
          <td>{{ m.baseSymbol }} / {{m.qouteSymbol}}</td>
          <td>
  ...
  methods: {
    ...
    getCoin() {
      ...
      Promise.all([
          ...
          api.getMarkets(id)
    ])  /// funcion nativa, permite enviar un array con diferentes promesas  y manejar la rrespuesta cuando todas terminen
    .then(
